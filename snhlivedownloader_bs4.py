...
def _get_ts_from_m3u8(m3u8_url):
    ...
    try:
        ...
        print("import requests, BeautifulSoup Error")
        sys.exit(1)
...
def _get_downloadable_from_url(video_url, resolution):
    ...
    video_html = BeautifulSoup(r.text,'html5lib')
    title = video_html.find('span',class_='title1').text
    info = video_html.find('span',class_='title2').text

    video_html = BeautifulSoup(r.text,'html5lib')
    chao_url = video_html.find('input',{'id':'chao_url'})['value']
    gao_url = video_html.find('input',{'id':'gao_url'})['value']
    liuchang_url = video_html.find('input',{'id':'liuchang_url'})['value']
...
def spider_snhLive():
    ...
    if SINGLE == '1':
        ...
        index_html = BeautifulSoup(r.text,'html5lib')

        # 获取页面总数
        num = int(index_html.find('a', class_='end').text)
        print("共%d页" % num)
        print("--------------------------------------------------------------")
        ...
        for i in range(PAGE_START,num+1):
            ...
            page_html = BeautifulSoup(r.text,'html5lib')

            page_list = page_html.findAll('li', class_='videos')
            ...
            for video in page_list:
                ...
                video_img = video.img['src']
                video_url = site_url + video.a['href']
                parsed = _get_downloadable_from_url(video_url, RESOLUTION)
